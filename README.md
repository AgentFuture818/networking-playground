...
GitHub Pages 靜態站，用書面**廣東話**教 computer networking。對象係香港**大專**：問題 → 日常 → 講解 → 玩。第一期只解鎖**單元 1「點樣搵到對方」**。地圖預告單元 2–16，內容鎖住。另開「伺服器維護」軌道佔位。
...
**可玩：** 單元 1 點樣搵到對方（問題圖 → 認五位 → IPv4 四格同 RFC 1918 → IPv6 用圖寫 16 byte → IPv6 範圍 → 先至虛擬 lab 送包 → 兩項練習）。後一節要上一節標記完成先至解鎖。
